...
function renderFavourites() {
  ...
  grid.innerHTML = favouriteRecipes
    .map(
      (recipe) => `
...
function updateStats() {
  ...
  const avgCalories =
    Math.round(
      favouriteRecipes.reduce((sum, recipe) => sum + recipe.calories, 0) /
        totalFavourites
    ) || 0;
...
function removeFromFavourites(recipeId) {
  ...
  if (
    confirm(
      `Are you sure you want to remove "${recipe.name}" from your favourites?`
    )
  ) {
