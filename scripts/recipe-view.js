...
function updateServingsDisplay() {
  ...
  const newCalories = Math.round(
    (recipeData.baseCalories * currentServings) / recipeData.baseServings
  );
...
function addToPlan() {
  ...
  showNotification(
    `${recipeData.name} added to your meal plan for ${currentServings} servings!`,
    "#00D2A0"
  );
}
