...
function attachEventListeners() {
  ...
  // Input fields
  document
    .getElementById("householdSize")
    .addEventListener("input", function () {
      answers["6"] = {
        householdSize: this.value,
...
function handleMultipleSelect(selectedOption, questionNumber, value) {
  ...
  if (selectedOption.classList.contains("selected")) {
    ...
    answers[questionNumber] = answers[questionNumber].filter(
      (item) => item !== value
    );
  }
  ...
  if (value === "none" && selectedOption.classList.contains("selected")) {
    ...
    value !== "none" &&
    selectedOption.classList.contains("selected")
  ) {
